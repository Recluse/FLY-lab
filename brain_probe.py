...
def run(args):
    ...
    if args.mode == "stream":
        ...
        selected = [r for r in rows if r["cell_class"] == args.cell_class and (r["cell_sub_class"] == args.sub_class or args.sub_class == "*") and (r["side"] == args.side or args.side == "both")]
        inputs = [int(r["root_id"]) for r in selected]
        if not inputs:
            raise ValueError(f"No annotated cells for {args.cell_class}/{args.sub_class}/{args.side}")
        save(out / "sensory_ids.json", {"materialization": 783, "selection": f"all cells with cell_class={args.cell_class} cell_sub_class={args.sub_class}; side is nerve-entry side", "annotation_commit": subprocess.check_output(["git", "-C", str(repo), "rev-parse", "HEAD"], text=True).strip(), "annotation_sha256": hashlib.sha256(table.read_bytes()).hexdigest(), "rows": selected})
    ...
    if missing:
        save(out / "blocked.json" if not args.allow_missing else out / "excluded_ids.json", {"reason": "verified sensory IDs absent from model completeness", "missing": missing, "excluded": bool(args.allow_missing)})
        if not args.allow_missing:
            raise ValueError(f"Missing {len(missing)} IDs; refusing silent exclusion")
        inputs = [v for v in inputs if v in mapping]
...
if __name__ == "__main__":
    ...
    p.add_argument("--cell-class", default="gustatory")
    p.add_argument("--sub-class", default="sugar/water", help="'*' = any sub class")
    p.add_argument("--allow-missing", action="store_true", help="drop annotated IDs absent from the model, recorded in excluded_ids.json")
