...
def body(args):
    ...
    brain = None
    if args.policy:
        from pilot import events_for, select_command, adapt, readout_turns, BRAIN_POLICIES
        events = events_for(args.scenario, args.seed, blocks)
        save(out / "events.json", events)
        if args.policy in BRAIN_POLICIES:
            args.brain_ticks = str(Path(args.brain_ticks).resolve())
            brain = json.loads(Path(args.brain_ticks).read_text())
            own_sha = hashlib.sha256((out / "events.json").read_bytes()).hexdigest()
            if args.policy == "D":
                assert brain["events_sha256"] != own_sha, "D replay must come from an episode with different events"
            else:
                assert brain["events_sha256"] == own_sha, "brain pass events differ from this episode's events"
            assert len(brain["ticks"]) == blocks and abs(brain["ticks"][-1]["t_end_s"] - blocks * .015) < 1e-9, "brain/body model time mismatch"
            turns = readout_turns(brain["ticks"], args.readout_gain, args.readout_deadband, args.readout_ema)
            save(out / "brain_source.json", {"brain_ticks": str(Path(args.brain_ticks).relative_to(ROOT)), "brain_sha256": hashlib.sha256(Path(args.brain_ticks).read_bytes()).hexdigest(),
                                             "readout": {"gain": args.readout_gain, "deadband": args.readout_deadband, "ema": args.readout_ema}, "turns": turns})
    try:
        for block in range(blocks):
            ...
            if events is not None:
                ...
                if brain is not None:
                    assert abs(brain["ticks"][block]["t_end_s"] - (block + 1) * .015) < 1e-9
                    command = select_command(args.policy, args.scenario, sensory, turns[block])
                else:
            ...
            if brain is not None:
                rows[-1].update(brain_dn_left=brain["ticks"][block]["DN_left"], brain_dn_right=brain["ticks"][block]["DN_right"])
    finally:
        if rows:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", choices=["A", "B", "C", "D", "E"])
    parser.add_argument("--brain-ticks", help="brain_loop brain_ticks.json for policies C/D/E")
    parser.add_argument("--readout-gain", type=float)
    parser.add_argument("--readout-deadband", type=float)
    parser.add_argument("--readout-ema", type=float)
    ...
    if args.policy in {"C", "D", "E"} and (not args.brain_ticks or None in (args.readout_gain, args.readout_deadband, args.readout_ema) or not 0 <= args.readout_ema < 1):
        parser.error("brain policies need --brain-ticks and readout gain/deadband/ema in range")
