...
class FlyReservoir:
    ...
    def __init__(self, shuffle_seed=None, random_seed=None, neurons_per_channel=40, tick_ms=15):
        ...
        self.channels = [left[:k], right[:k], left[k:2 * k]]  # bits: value, positive-in-unsat, negative-in-unsat
        ...
        self.wiring = {"materialization": 783, "shuffle_seed": shuffle_seed, "random_seed": random_seed, "channels": "head bristle left[:k], right[:k], left[k:2k]", "neurons_per_channel": k,
    ...
    def __call__(self, obs):
        for bit, ch in zip(obs, self.pois):
            for p in ch:
                p.active = bool(bit)
        ...
        self.prev = counts
        return new[self.dn] / 10.  # ~unit scale
...
def main(args):
    ...
    tag = f"{args.agent}-s{args.seed}" + ("-shaped" if args.shaping else "")
